...
define(["vue","jquery","util"], function (Vue, $, Util) {
    'use strict'
    var DC = Util
    DC.ui = {}
    DC.ui.count = 0
    DC.ui.comps = {}
    DC.ui.remove = function(id) {
        DC.ui.comps[id] = undefined
        DC.ui.comps[id] = null
        delete DC.ui.comps[id]
    }
    DC.ui.get = function(id) {
        return DC.ui.comps[id]
    }
    DC.ui.add = function(id, value) {
        DC.ui.comps[id] = value
    }
    DC.ui.get_id = function(id) {
        if (!id) {
            id = 'DCui-' + DC.ui.count
            DC.ui.count++
        }
    ...
    DC.ui.affix = function(fix_pos, fix_comp, max_top) {
        var intveral = null
        var time_span = 100
        var scroll = function() {
    ...
    DC.ui.unmask = function() {
        $('.shadow').remove()
    }
    DC.ui.mask = function(config) {
        config = config || {}
        if (!config.multi && $('.shadow').length > 0) {
            return
        }
        var mask = '<div class="shadow"><i class="fa fa-refrDCh fa-spin"></i></div>'
        if (!config.ele) {
            mask = $(mask)
            mask.css({
    ...
    DC.ui.popup = function(config) {
        config = config || {}
        config = DC.util.merge({
            title: DC.msg.get('default_title'),
        ...
        var render = function() {
            $('.popup').remove()
            var minHeight = DC.client_height - 70
            var minWidth = 300
            var top = (DC.client_height - minHeight) / 2 - 10
            var left = (DC.client_width - minWidth) / 2
            ...
            if (!config.auto_hide && config.auto_hide_default && !config.hide_buttons) {
                config.innerHTML += '<div class="popup-btn">' +
                    '<div style="margin:auto;float: none;"><a href="javascript:void(0);" class="btn btn-major btn-close">' + DC.msg.get('close') + '</a></div></div>'
            }
            if (config.innerHTML) {
            ...
            var close_func = function(e) {
                ...
                if (config.before_callback) {
                    var rDCult = config.before_callback()
                    if (!rDCult) {
                        return
                    }
                }
            ...
            if (config.center) {
                ...
                popup.css({
                    top: ((DC.client_height - height) / 2) + 'px',
                    left: ((DC.client_width - width) / 2) + 'px'
                })
            }
            if (config.auto_hide) {
            ...
            $('.popup-title').dblclick(function(){
                ...
                if(w <= DC.client_width) {
                    pop.css({
                        top:0,
                        left:0,
                        width:DC.client_width,
                        height:DC.client_height
                    })
                } else {
                    w = pop.data('value')
                    pop.css({
                    ...
                    DC.ui.popup.auto_position()
                }
            })
        }
        render();
    }
    DC.ui.popup.auto_position = function() {
        var div = $('.popup')
        if (div.length <= 0) {
            return
        }
        DC.client_height = document.documentElement.clientHeight
        DC.client_width = document.documentElement.clientWidth

        var height = div.outerHeight()
        var width = div.outerWidth()

        var top = (DC.client_height - height) / 2 - 10
        var left = (DC.client_width - width) / 2
        ...
        if(popup_common && popup_common.length > 0){
            if(DC.client_height<height){
                popup_common.css('max-height',(height-70)+'px')
                popup_common.css('height',(DC.client_height-70)+'px')
            }
        }
    }
    DC.ui.confirm = function(config, succDCs, fail) {
        config = config || {}
        config = DC.util.merge({
            innerHTML: ''
        }, config)
        ...
        var positiveBtnMsg = config.positiveBtnMsg || DC.msg.get('confirm')
        var cancelBtnMsg = config.cancelBtnMsg || DC.msg.get('cancel')
    ...
    DC.ui.alert = function(config,isHyperlink) {
        config = config || {}
        config = DC.util.merge({
            title: DC.msg.get('default_title'),
            auto_hide: true
        ...
        if(isHyperlink){
            var mDCsage  = '<p ' + cls + '><a href="'+DC.deploy+config.hyperlink+'">' + msg + '</a></p>'
        } else {
            var mDCsage  = '<p ' + cls + '>' + msg + '</p>'
        }
    ...
    DC.ui.input = function(config) {
        ...
        DC.ui.add(ret.id, ret)
        return ret
    }

   /* DC.ui.input = function(config) {
        var DC_input = function(conf) {
            conf = conf || {}
            if (!conf.el){
                return
            }
            ...
            var configCn = {
                dateFormat: 'yy-mm-dd',
                yearSuffix: '年',
                monthNamDC: ['一月','二月','三月','四月','五月','六月','七月','八月','九月','十月','十一月','十二月'],
                dayNamDC: ['星期日','星期一','星期二','星期三','星期四','星期五','星期六'],
                dayNamDCShort: ['周日','周一','周二','周三','周四','周五','周六'],
                dayNamDCMin: ['日','一','二','三','四','五','六']
            }
            if(DC.lang === 'zh-cn') {
                conf = DC.util.merge(conf, configCn)
            }
            if (el.hasClass('date')) {
            ...
            if (el.hasClass('datemonth')) {
                conf = DC.util.merge(conf, {
                    monthNamDC: ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12'],
                    monthNamDCShort: ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12'],
                    changeMonth: true,
                    changeYear: true,
            ...
            if (el.hasClass('datetime')) {
                conf = DC.util.merge(conf,{
                    dateFormat: "yy-mm-dd",
                    timeText: DC.msg.get('timepicker_timeText'),
                    hourText: DC.msg.get('timepicker_hourText'),
                    minuteText: DC.msg.get('timepicker_minuteText'),
                    //secondText: DC.msg.get('timepicker_secondText'),
                    closeText: DC.msg.get('timepicker_closeText'),
                    currentText: DC.msg.get('timepicker_currentText'),
                    timeInput: true,
                    timeFormat: "HH:mm:00",
                    showHour: false,
                    showMinute: false
            ...
            if (el.attr('type') != 'hidden') {
                ...
                var icon = conf.icon ? DC.util.format_string(DC.tpl.icon_tpl, {
                ...
                if (!config.msg) {
                    config.msg = DC.msg.get('non_empty')
                }
                ...
                if (label) {
                    ...
                    if (el.attr('type') != 'checkbox' && el.attr('type') != 'radio') {
                        parent.prepend(DC.util.format_string(DC.tpl.label_tpl, {
                            display: label,
                            id: conf.el
                        }))
                    } else {
                        parent.append(DC.util.format_string(DC.tpl.label_tpl, {
                            display: label,
                            id: conf.el
                        }))
                    }
                ...
                if (after_labels) {
                    DC.each(after_labels, function(_, v) {
                ...
                this.validate = function() {
                    ...
                    if (!this.get_value()) {
                        if(el.hasClass('number')){
                            ...
                            /!*else if(this.el.val()!='' && this.get_value()== 0){
                             this.set_msg(config.default_number_msg || DC.msg.get('more_than_zero'))
                             rDCult = false
                             }*!/
                    ...
                    if (rDCult && this.get_value() && this.mask) {
                        ...
                        if (!reg.tDCt(v)) {
                            if (mask.msg){
                                ...
                                this.set_msg(DC.msg.error_regex)
                            }
                        } else {
                            this.clear_invalid()
                        }
                    ...
                    if (rDCult && this.customer_validate) {
                        rDCult = this.customer_validate()
                    }
                    return rDCult
                }
                el.on('change', function(e) {
                    var v = $(this).val()
                    var _input = DC.ui.get(e.target.id)
                    ...
                    if (comp) {
                        ...
                        if (!reg.tDCt(v)) {
                            ...
                            if (comp.msg){
                                ...
                                _input.set_msg(DC.msg.error_regex)
                            }
                            e.preventDefault()
                        } else {
            ...
            if (el.hasClass('number')) {
                ...
                if(el.hasClass('date') || el.hasClass('datetime')){
                    el.on('change', function() {
                        ...
                        if(!DC.util.judge_date_interval(minDate,maxDate,v,el.hasClass('date'))){
                            $(this).val('')
                        }
                        if(el.hasClass('date')) {
            ...
            this.set_value = function(v) {
                return this.el.val(DC.util.xssunDCcape(v))
            }
            this.set_nonempty = function(nonempty) {
                var label = this.el.closDCt('.input-wrap').find('label')
                if (label) {
                    this.nonempty = nonempty
                    if (nonempty) {
            ...
            this.clear_invalid = function() {
                var tip_msg = this.el.closDCt('div').find('.msg-tip')
                if (tip_msg.length > 0){
                    tip_msg.remove()
                }
                var error_style = config.msg_style || 'error'
                this.el.removeClass('DC-input-invalid')
                this.el.removeClass(error_style+'_frame')
                DC.get('.main-popup-error').hide().children().hide()
                function findMsg(el){
            ...
            this.set_msg = function(msg) {
                ...
                if (this.msg_type && this.msg_type == 'tips') {
                    ...
                    DC.get(tips).insertBefore(this.el)
                } else if(this.msg_type && this.msg_type == 'popup-tips'){
                    //错误类型 默认error warn
                    var error_style = config.msg_style || 'error'
                    var i_cls = 'fa-timDC-circle'
                    switch (error_style){
                        case 'warn':
                    ...
                    DC.get('.main-popup-error').children().hide()
                    DC.get('.main-popup-error').show().find('.popup-'+error_style).html('<i class="fa '+i_cls+'"></i> ' + msg).show()
                    DC.get('#'+this.id).addClass(error_style+'_frame')
                }else{
                    function findMsg(el){
                        if(el.parent().children('.msg').length==0){
                            findMsg(el.parent())
    ...
    DC.ui.select = function(config) {
        var ret = DC.ui.input(config)
        ret.bind_list = function(list) {
            this._list = list
            var out = []
            for (var i = 0, l = list.length; i < l; i++) {
                out.push(DC.util.format_string(DC.tpl.option_tpl, list[i]))
            }
        ...
        ret.get_display_value = function() {
            return DC.get('#' + config.el + ' option:selected').text()
        }
        if (config.disable){
    ...
    DC.ui.input_date_input = function(config) {
        config.cls = "input_date_input"
        var from_ret = DC.ui.input(config)
        var toInput = DC.get('<input type="text" name="'+config.toName+'" id="toDate-'+config.toName+'" class="date" style="padding-left: 10px;"/>')
        ...
        if(DC.lang === 'zh-cn') {
            config = DC.util.merge(config, configCn)
        }
        ...
        var change_input_date = function() {
            ...
            //判断minDate maxDate v 是否在该区间
            if(!DC.util.judge_date_interval(minDate,maxDate,v,true)){
                $(this).val('')
            }
        }
        ...
        from_ret.set_msg = function(msg) {
            ...
            if (config.msg_type && config.msg_type == 'tips') {
                if(from_ret.el.val() == ''){
                    var tips = '<div class="msg-tip error">' + msg + '</div>'
                    DC.get(tips).insertBefore(from_ret.el)
                }
                if(toInput.val() == ''){
                    var tips = DC.get('<div class="msg-tip error">' + msg + '</div>')
        ...
        from_ret.validate = function() {
            ...
            if (rDCult && from_ret.customer_validate) {
                rDCult = from_ret.customer_validate()
            }
            return rDCult
        }

        from_ret.etype = 'input_date_input'
        DC.ui.add(from_ret.id, from_ret)
        return from_ret
    }

    DC.ui.input_year_month = function(config) {
        config.cls = "input_year_month"
        var from_ret = DC.ui.select(config)
        var toInput = DC.get('<select name="'+config.toName+'" id="toDate-'+config.toName+'" style="padding-left: 10px;"></select>')
        toInput.insertAfter(from_ret.el.next())
        ...
        for (var i = 0, l = monthList.length; i < l; i++) {
            out.push(DC.util.format_string(DC.tpl.option_tpl, monthList[i]))
        }
    ...
    DC.ui.input_date_time = function(config) {
        var ret = DC.ui.input(config)
        config.dateFormat = "yy-mm-dd"
        // 中文显示
        var configCn = {
        ...
        var time = DC.get('<select class="input-date-time input-time"></select>')
        var out = []
        for (var i = 0; i < 24; i++) {
            var d = i < 10 ? '0' + i : '' + i
            out.push(DC.util.format_string(DC.tpl.option_tpl, {
                display: d + ':00:00',
                value: d + ':00:00'
            }))
        }
    ...
    DC.ui.input_time = function(config) {
        var hour = DC.ui.select(config)
        var out = []
        for (var i = 0; i < 24; i++) {
            var d = i < 10 ? '0' + i : '' + i
            out.push({
        ...
        var second = DC.get('<select class="input-time-second"></select>')
        var minute = DC.get('<select class="input-time-minute"></select>')
     ...
     * @param  {String} config.module  港口信息区分[内贸整箱、外贸整箱、外贸拼箱]---both:全部；DomDCticShippingSpu:内贸；ForeignTrunkSpu:外贸；LclSpu:拼箱
     * @param  {String} config.type  标识起始港还是目的港---from:查找起始港港口；to:查找目的港港口
     * @param  {String} config.param 其他参数
     */
    DC.ui.input_port = function(config) {
        var ret = DC.ui.input(config)
        ...
        ret.set_value = function(val) {
            var g = this
            var find_geo_callback = function(rDC) {
                if (rDC.ports.length <= 0) {
                    return
                }
                rDC = rDC.ports[0]
                if (DC.lang == 'en-us') {
            ...
            DC.util.ajax_get('/geo/find_port_by_code', {
                code: val
            }, find_geo_callback)
        ...
        var filter_geo = function(value) {
            ...
            var port = ''
            if (related && DC.ui.get(related)) {
                port = DC.ui.get(related).get_value()
            }
            var width = input.outerWidth() - 22
            width = width <= 200 ? 200 : width
            var height = input.outerHeight()
            var offset = input.offset()
            var poupFlag = input.closDCt('.popup').length>0?true:false
            ...
            drop.empty().html('<div class="input-port-item" data-value="-1">' + DC.msg.get('loading') + '</div>').show()
            var reqData = DC.util.merge({
                q: inputValue,
                port: port,
                type: module
            }, ret.param)
            DC.util.ajax_get(ws,reqData , function(rDC) {
                var out = []
                var selected = false
                var preference = DC.ui.get(input.attr('id')).preference
                if(preference) {
                    out.push('<div class="input-port-sep" data-value="-1">' + DC.msg.get('preference') + '</div>')
                    DC.each(preference,function(_,v){
                        if (DC.lang == 'en-us') {
                ...
                if (!rDC.ports || rDC.ports.length <= 0) {
                    out.push('<div class="input-port-item" data-value="-1">' + DC.msg.get('default_empty_rDCult') + '</div>')
                    input.data('value', '')
                } else {
                    var filter = {}
                    var ports = rDC.ports
                    DC.each(ports, function(_, v) {
                        filter[v.code] = v
                    })
                    var t_p = []
                    for (var p in filter) {
                ...
                drop.children('div.input-port-item').on('click', function(e) {
                    ...
                    DC.event.fire(v.parent().data().value, 'change', v.data().value)
                    v.parent().hide()
                    e.preventDefault()
                })
            })
        ...
        if(window.attachEvent && document.getElementById(ret.id).onpropertychange){
            ...
            DC.get('#' + ret.id).on('input',function(){
                if(inp){
                    DC.get('#' + ret.id).data().value = ''
                    if (intveral != null) {
                        clearTimeout(intveral)
        ...
        DC.get('#' + ret.id).on('keydown', function(evt) {
            inp = null
            ...
            DC.get(this).trigger('focus')
            e.preventDefault()
            return false
        }).on('focus', function(e) {
            var c = DC.get('#' + DC.get(this).attr('id') + '-drop')
            if (!c.is(":visible")) {
                c.show()
                var c = DC.get(this)
                var v = c.data('value')
    ...
    DC.ui.input_city = function (config) {
        config.value_field = function (rDC) {
            return rDC.id
        }
        config.display_field = function (rDC) {
            return rDC.name
        }
        config.filter = function (list, value) {
            var ret = []
            DC.each(list, function (_, v) {
        ...
        var filter = DC.ui.input_filter(config)

        DC.util.ajax_get('/geo/query_all_city', {}, function (rDC) {
            filter.load_data(rDC.children)
        })

        return filter
    }

    DC.ui.input_filter = function(config) {
        var ret = DC.ui.input(config)
        ...
        ret.set_value = function(val) {
            if(ret.data.length>0){
                var has_val = false
                DC.each(ret.data,DC.delegate(function(_,v){
        ...
        var filter = function() {
            ...
            var drop = $('#' + input.attr('id') + '-drop')
            var par = DC.ui.get(input.attr('id'))
            ...
            if (par.data.length <= 0) {
                if(allow_enter){
                    ...
                    drop.html('<div>' + DC.msg.get('default_empty_rDCult') + '</div>').show()
                }
            }
            var rDC = (!value && !display) ? par.data : par.filter(par.data, display, value)
            if (rDC.length <= 0) {
                if(allow_enter){
                    drop.html('').hide()
                } else {
            ...
            drop.html(out.join('')).show().children('div').on('click', function(e) {
                ...
                DC.event.fire(id, 'change', v.data().value)
                v.parent().hide()
                e.preventDefault()
            })
        }
        ...
        if(window.attachEvent && document.getElementById(ret.id).onpropertychange){
            ...
            DC.get('#' + ret.id).on('input',input_listener)
        }
        ...
        var scrollCalculate = function(){
            ...
            if (selectedTop - dropTop + selectedOutHeight <= 1) {
                var height = 0
                DC.each(DC.get('.input-item-selected').prevAll(), function (_, item) {
                    height += $(item).outerHeight()
                })
                DC.get('.input-drop').scrollTop(height)
            } else if (selectedTop - dropTop + selectedOutHeight > dropOutHeight) {
                var height = 0
                DC.each(DC.get('.input-item-selected').prevAll(), function (_, item) {
                    height += $(item).outerHeight()
                })
                DC.get('.input-drop').scrollTop(height + selectedOutHeight - dropOutHeight)
            }
        }

        DC.get('#' + ret.id).on('keyup', function(evt) {
            if (evt.which == 13) {
                // enter
            ...
            // if (DC.get('#' + ret.id).val().trim() == '') {
            // 	DC.ui.get(ret.id).clear_value();
            // }
            ...
            if (evt.which == 9) {
                // tab
                if(DC.get('#' + ret.id).val() && drop[0].style.display!='none') {
                    drop.children('div').eq(0).trigger('click')
                } else {
                    drop.hide()
                }
    ...
    DC.ui.geo_selector_v2 = function(config) {
        var ret = DC.ui.select(config)
        ...
        var filter_geo = function(comp, level, val, set_val, is_inited) {
            ...
            DC.util.ajax_get(query, param, function(rDC) {
                if(has_set_value && is_inited) return
                var list = rDC.children
                if (list.length < 1) {
                    var parent_comp = comp.parent()
                    comp.remove()
                    rDCet_css(parent_comp.children('select'))
                    return
                }
                var out = [DC.util.format_string(DC.tpl.option_tpl, {
                    display: DC.lang == DC.consts.language.ZH_CN ? '请选择' : 'Please Select...',
                    value: -1
                })]
                for (var i = 0, l = list.length; i < l; i++) {
                    out.push(DC.util.format_string(DC.tpl.option_tpl, {
                ...
                comp.on('change', function() {
                    ...
                    if (index + 1 <= to_level) {
                        ...
                        if (v == '-1') {
                            rDCet_css(c.parent().children('select'))
                            return false;
                        }
                        var s = DC.get('<select data-value="' + (index + 1) + '"></select>')
                        s.insertAfter(c)
                        rDCet_css(c.parent().children('select'))
        ...
        var rDCet_css = function(child_list) {
            if(ratio){
        ...
        ret.set_value = function(v) {
            ...
            if (v.length < 1) {
                ...
                for (var i = 0; i < v.length - 1; i++) {
                    if (i > 0) {
                        s = DC.get('<select data-value="' + i + '"></select>')
                        s.insertBefore(msg)
                    }
                    filter_geo(s, this.from_level + i, v[i], v[i + 1], false)
                }
            }
            rDCet_css(this.el.parent().children('select'))
        }
        ret.clear_value = function() {
    ...
    DC.ui.geo_selector = function(config) {
        var ret = DC.ui.input(config)
        ...
        var show_geo = function() {
            ...
            var tab_panel = DC.ui.get(geo.attr('id') + '-drop')
            tab_panel.el.css({
        ...
        var filter_geo = function(tab_panel, level, val) {
            ...
            DC.util.ajax_get(query, {
                parent: val
            }, function(rDC) {
                var out = ['<ul>']
                DC.each(rDC.children, function(_, v) {
                    out.push('<li data-value="' + v.id + '">' + v.name + '</li>')
                })
                out.push('</ul>')
                tab_panel.set_panel_dom(level, out.join(''))

                DC.get('#' + tab_panel.id + ' .panel').eq(level).children('ul').children('li').on('click', function() {
                    var li = $(this)
                    li.parent().children('li').removeClass('selected')
                    li.addClass('selected')
                    var value = li.data('value')
                    var lev = li.closDCt('.panel').index()
                    var drop = li.closDCt('.input-geo-drop')
                    lev = (+lev) + 1
                    ...
                    DC.get('#' + drop.data('value')).val(get_selected_string(drop)).data('value', value)

                    if (lev >= 5) {
                        drop.hide()
                        return
                    }
                    var tab_panel = DC.ui.get(drop.attr('id'))
                    filter_geo(tab_panel, lev, value)
                })
            })
        }
        DC.get('#' + ret.id).on('click', function(e) {
            if (DC.get(this).hasClass('disabled')) {
                e.preventDefault()
                return false
            }
            show_geo.apply(this)
            filter_geo(DC.ui.get($(this).attr('id') + '-drop'), 0, null)
            e.preventDefault()
            return false
        })
        ret.etype = 'geo_selector'
    ...
    DC.ui.input_form = function(config) {
        var DC_input_form = function(conf) {
            conf = conf || {}
            if (!conf.el){
                return
            }
            ...
            $.each(items_dom, function(i, v) {
                ...
                if (conf.items[item_index].type == 'select') {
                    input = DC.ui.select(conf.items[item_index])
                } else if (conf.items[item_index].type == 'geo') {
                    input = DC.ui.geo_selector(conf.items[item_index])
                } else if (conf.items[item_index].type == 'geo_v2') {
                    input = DC.ui.geo_selector_v2(conf.items[item_index])
                } else if (conf.items[item_index].type == 'port') {
                    input = DC.ui.input_port(conf.items[item_index])
                } else if (conf.items[item_index].type == 'filter') {
                    input = DC.ui.input_filter(conf.items[item_index])
                } else if (conf.items[item_index].type == 'qty') {
                    input = DC.ui.input_qty(conf.items[item_index])
                } else if (conf.items[item_index].type == 'date_time') {
                    input = DC.ui.input_date_time(conf.items[item_index])
                } else if (conf.items[item_index].type == 'time') {
                    input = DC.ui.input_time(conf.items[item_index])
                } else if (conf.items[item_index].type == 'file') {
                    input = DC.ui.file(conf.items[item_index])
                } else if (conf.items[item_index].type == 'btn') {
                    input = DC.ui.button(conf.items[item_index])
                } else if (conf.items[item_index].type == 'date_input') {
                    input = DC.ui.input_date_input(conf.items[item_index])
                } else if (conf.items[item_index].type == 'year_month') {
                    input = DC.ui.input_year_month(conf.items[item_index])
                } else {
                    input = DC.ui.input(conf.items[item_index])
                }
                items.push(input)
                item_index++
            })
            ...
            this.validate = function() {
                ...
                for (var i = 0; i < this.items.length; i++) {
                    if(this.items[i].msg_type=='popup-tips'){
                        ...
                        rDCult =  this.items[i].validate() && rDCult
                    }
                }
                if (rDCult && this.customer_validate) {
                    rDCult = this.customer_validate()
            ...
            this.get_value = function() {
                ...
                for (var i = 0; i < this.items.length; i++) {
                    ...
                    if(this.items[i].etype == 'input_date_input'){
                        rDC = DC.util.merge(rDC,this.items[i].get_value())
                    }else{
                        rDC[name] = this.items[i].get_value()
                    }
                }
                return rDC
            }
            ...
            this.get_item_by_id = function(id) {
                var rDC = null
                DC.each(this.items, function(_, v) {
        ...
        var ret = new DC_input_form(config)
        DC.ui.add(ret.id, ret)
        return ret
    }

    DC.ui.button = function(config) {
        var DC_button = function(conf) {
            conf = conf || {}
            if (!conf.el){
                return
            }
        ...
        var ret = new DC_button(config)
        DC.ui.add(ret.id, ret)
        return ret
    }

    DC.ui.input_qty = function(config) {
        var DC_input_qty = function(conf) {
            conf = conf || {}
            conf = DC.util.merge({
                max_value: 99999,
                min_value: 0,
            ...
            el.parent().children('.input-qty-minus').on('click', function() {
                ...
                qty.val(+qty.val() - 1)
                DC.event.fire(qty.attr('id'), 'change', qty.val())
                qty.trigger("change");
            })
            ...
            el.on('change', function(e) {
                var v = +$(this).val()
                v = Math.ceil(v)
                var comp = DC.ui.get(e.target.id)
                var change_val = v
                ...
                DC.event.fire($(this).attr('id'), 'change', change_val)
            })
            ...
            this.set_msg = function(msg) {
                ...
                if (conf.msg_type && conf.msg_type == 'tips') {
                    ...
                    this.el.parent().children('.msg').addClass('error').html('<i class="fa fa-timDC-circle"></i> ' + msg).show()
                }
            }
        ...
        var ret = new DC_input_qty(config)
        DC.ui.add(ret.id, ret)
        return ret
    }

    DC.ui.tab_h = function(config) {
        var DC_tab_h = function(conf) {
            conf = conf || {}
            if (!conf.el){
                return
            }
            ...
            DC.get(id + ' .tab').on('click', function() {
                var last_index = $(this).parent().children('.current').data().index
                $(this).parent().children().removeClass('current')
                var index = $(this).addClass('current').data().value
                var list = DC.get(id + ' .panel')
                list.hide()
                list.eq(+index).show()
                DC.event.fire(conf.el, 'tab-change', [list.eq(+last_index), list.eq(+index)])
            })

            DC.get(id + ' .panel').eq(0).show()
        }
        var ret = new DC_tab_h(config)
        DC.ui.add(ret.id, ret)
        return ret
    }

    DC.ui.tab_v = function(config) {
        var DC_tab_v = function(conf) {
            conf = conf || {}
            if (!conf.el){
                return
            }
            ...
            this.set_panel_dom = function(index, dom) {
                var id = '#' + this.id
                DC.get(id + ' .panel').eq(index).html(dom)
            }

            this.set_panel = function(index) {
                var id = '#' + this.id
                DC.get(id + ' .tab').eq(index).trigger('click')
            }
        }
        var ret = new DC_tab_v(config)
        DC.ui.add(ret.id, ret)
        return ret
    }

    DC.ui.date_selector = function(config) {
        var DC_date_selector = function(conf) {
            conf = conf || {}
            if (!conf.el){
                return
            }
            ...
            this.set_date = function(d) {
                ...
                date.push({
                    ...
                    style: 'width:15px;',
                    parent: this.id,
                    value: DC.util.date_to_string(d),
                    content: '<i class="fa fa-caret-left"></i>'
                })
                for (var i = 0; i < 7; i++) {
                    d.addDate(1)
                    var value = DC.util.date_to_string(d)
                    var date_display = DC.util.date_to_string(d, '{{m}}/{{d}}')
                    d.addDate(6)
                    date_display += ' - ' + DC.util.date_to_string(d, '{{m}}/{{d}}')
                    date.push({
                        cls: '',
                ...
                this.el.children('.date-selector').children('ul').children('li').on('click', function() {
                    ...
                    if (!obj.hasClass('date-selector-item-arrow')) {
                        DC.ui.get(parent).set_current(index - 1)
                    } else {
            ...
            this.set_current = function(index) {
                ...
                DC.event.fire(this.id, 'change', li.data().value)
            }
            this.set_date(config.date)
        }
        var ret = new DC_date_selector(config)
        DC.ui.add(ret.id, ret)
        return ret
    }

    DC.ui.paging = function(config) {
        var pagDCize = $.cookie('pagDCize')
        var setCookie = !pagDCize ? false : true
        if (!pagDCize){
            pagDCize = 25
        }
        DC.pagDCize = pagDCize
        var DC_paging = function(conf) {
            conf = conf || {}
            if (!conf.el){
                return
            }
            ...
            this.refrDCh = function(total_page, page, total_count) {
                this.current = page
                var paging = $('#' + this.id)
                var from = page - 2
                var end = page + 2
                ...
                var inner = []
                if (page > 1) {
                    inner.push(DC.util.format_string(itemTpl, {
                ...
                inner.push('<span class="paging-info"> ' + DC.msg.get('common_paging_recordnum', {arg0: ' ' + total_count + ' '}) + ' / ' + total_page + ' ' + DC.msg.get('common_paging_pagenum') + '</span>')
                if(this.show_per_page){
                    inner.push('&nbsp;<span class="paging-info"> ' + DC.msg.get('per_page') +'：<a>25</a>，<a>50</a>，<a>100</a></span>')
                }
                ...
                $('#' + this.id + ' .paging-item').click(function() {
                    ...
                    if (page > 0) {
                        DC.event.fire(id, 'change', page)
                    }
                })
                if(this.show_per_page){
                    var set_pagDCize_link = $('#' + this.id + ' .paging-info a')
                    set_pagDCize_link.each(function(_,v){
                        if(DC.get(v).html() == DC.pagDCize){
                            DC.get(v).addClass('active')
                        }
                    })
                    set_pagDCize_link.click(DC.delegate(function(e) {
                        var pagDCize = $(e.target).html()
                        DC.pagDCize = pagDCize
                        $.cookie('pagDCize', DC.pagDCize, {
                            expirDC: 365,
                            path: '/'
                        })
                        DC.event.fire(this.id, 'change', 1)
                    },this))
                }
            }
            this.current = 1
        }
        var ret = new DC_paging(config)
        DC.ui.add(ret.id, ret)
        return ret
    }

    DC.ui.map = function(el, rDC, mapType) {
        var script = document.createElement("script");
        script.type = "text/javascript";
        script.src = "http://api.map.baidu.com/api?v=2.0&ak=GOEGRcwYZBAXOlU7a0bwf9DV&callback=trackingCallback";
        document.body.appendChild(script);
        var data=rDC.movements;
        ...
        window.trackingCallback = function() {
            ...
            var vectorFCArrow = new BMap.Marker(new BMap.Point(currentPoint.lng-0.01,currentPoint.lat), {
                ...
                icon: new BMap.Symbol(BMap_Symbol_SHAPE_FORWARD_CLOSED_ARROW, {
                    scale: 1.5,
                    strokeWeight: 1,
                    rotation: data[data.length-1].vDCselHead,//顺时针旋转30度
                    fillColor: 'red',
                    fillOpacity: 0.8
                })
            });
            ...
            var temp ='<table style="font-size: 13px;text-align: left" class="map-table">'+
                '<tr><td>'+DC.msg.get('call_sign')+'：{{{callNo}}}</td><td>'+DC.msg.get('bow_to')+'：{{{vDCselHead}}}℃</td></tr>'+
                '<tr><td>MMSI：{{{mmsi}}}</td><td>'+DC.msg.get('track_direction')+'：{{{vDCselCourse}}}℃</td></tr>'+
                '<tr><td>IMO：{{{imo}}}</td><td>'+DC.msg.get('speed')+'：{{{vDCselSpeed}}}kn</td></tr>'+
                '<tr><td>'+DC.msg.get('carrier')+'：{{{company}}}  </td><td>'+DC.msg.get('latitude')+'：{{{shipLatitude}}}</td></tr>' +
                '<tr><td>'+DC.msg.get('english_name')+'：{{{nameEn}}}    </td><td>'+DC.msg.get('longitude')+'：{{{shipLongitude}}}</td></tr>' +
                '<tr><td> '+DC.msg.get('chinDCe_name')+'：{{{nameCn}}}</td><td>'+DC.msg.get('fnd')+'：{{{etaPortName}}}</td></tr>'+
                '<tr><td>'+DC.msg.get('beam')+'：{{{width}}}m      </td><td>'+DC.msg.get('last_time')+'：{{{vDCselPositionTime}}}</td></tr>' +
                '<tr><td>  '+DC.msg.get('ship_length')+'：{{{length}}}m</td></tr>'+
                '</table>'

            var infoWindow = new BMap.InfoWindow(Handlebars.compile(temp)(DC.util.merge(rDC,data[data.length-1])), {
                minWidth :300,     // 信息窗口宽度
                minHeight: 140,     // 信息窗口高度
                enableMDCsage:false//设置允许信息窗发送短息
            });  // 创建信息窗口对象
    ...
    DC.ui.newMap = function(el, rDCult) {
        var script = document.createElement("script");
        script.type = "text/javascript";
        script.src = "http://api.map.baidu.com/api?v=2.0&ak=GOEGRcwYZBAXOlU7a0bwf9DV&callback=trackingCallback";
        document.body.appendChild(script);
        var data=rDCult.movements;
        var rDC = rDCult.vDCselTraceInfoDto
        ...
        window.trackingCallback = function() {
            ...
            var translateCallback = function (data){
                if(data.status === 0) {
                    ...
                    map.addOverlay(vectorFCArrow);
                    var temp ='<table style="font-size: 13px;text-align: left" class="map-table">'+
                        '<tr><td>'+DC.msg.get('english_name')+'：{{{name}}} </td><td>MMSI：{{{MMSI}}}</td></tr>'+
                        '<tr><td> '+DC.msg.get('chinDCe_name')+'：{{{cnName}}}</td><td>IMO：{{{IMO}}}</td></tr>'+
                        '<tr><td>'+DC.msg.get('call_sign')+'：{{{callSign}}}</td><td>'+DC.msg.get('latitude')+'：{{{lat}}}</td></tr>'+
                        '<tr><td>'+DC.msg.get('nation_owner')+'：{{{nationAndOwner}}}  </td><td>'+DC.msg.get('longitude')+'：{{{lon}}}</td></tr>' +
                        '<tr><td>'+DC.msg.get('vDCsel_type')+'：{{{type}}}    </td><td>'+DC.msg.get('heading_course')+'：{{{headingAndCourse}}}</td></tr>' +
                        '<tr><td> '+DC.msg.get('status_speed')+'：{{{statusAndSpeed}}}</td><td>'+DC.msg.get('length_beam')+'：{{{lengthAndBeam}}}</td></tr>'+
                        '<tr><td>'+DC.msg.get('fnd')+'：{{{dDCt}}}      </td><td>'+DC.msg.get('eta')+'：{{{ETA}}}</td></tr>' +
                        '<tr><td>'+DC.msg.get('draught')+'：{{{draught}}}      </td><td>'+DC.msg.get('last_time')+'：{{{lastTime}}}</td></tr>' +
                        '<tr><td>'+DC.msg.get('gross_ton')+'：{{{grossTon}}}      </td><td>'+DC.msg.get('load_ton')+'：{{{loadTon}}}</td></tr>' +
                        '<tr><td>'+DC.msg.get('net_ton')+'：{{{netTon}}}      </td><td>'+DC.msg.get('build_time')+'：{{{buildTime}}}</td></tr>' +
                        '</table>'

                    var infoWindow = new BMap.InfoWindow(Handlebars.compile(temp)(rDC, {
                        minWidth :300,     // 信息窗口宽度
                        minHeight: 140,     // 信息窗口高度
                        enableMDCsage:false//设置允许信息窗发送短息
                    }));  // 创建信息窗口对象
            ...
            var pointArr = [];
            pointArr.push(new BMap.Point(rDC.lonConverted, rDC.latConverted));
            convertor.translate(pointArr, 1, 5, translateCallback)
        }
    }


    DC.ui.slider = function(id, itemWidth, itemHeight, tailOffset, transType) {
        if(!transType) {
            transType = 'move'
        }
        var ret = {}
        ...
        ret.el = div
        ret.rDCize = function(width , height) {
            var div = this.el
        ...
        for (var i = 0; i < itemCount; i++) {
            pagDC.push('<span class="slider-selector">&nbsp;</span>')
        }
        div.children('.slider-pagDC').html(pagDC.join(''))
        DC.get(id + ' .slider-selector').eq(0).addClass('slider-selected')
        ...
        ret.move = function (index) {
            ...
            DC.get(id + ' .slider-selector').removeClass('slider-selected').eq(index).addClass('slider-selected')
            intervelSpan()
        }
        var current = 0
        var total = itemCount - 1
        DC.get(id + ' .slider-left').click(function (e) {
            if (current > 0) {
                current--
                ret.move(current)
                duration = 'left'
        ...
        DC.get(id + ' .slider-right').click(function (e) {
        ...
        DC.get(id + ' .slider-selector').click(function (e) {
            var index = DC.get(this).index()
            current = index
            ret.move(current)
            e.preventDefault()
            return false
    ...
    DC.ui.initSlider = function (id, itemWidth, itemHeight, tailOffset, transType) {
        return DC.ui.slider(id, itemWidth, itemHeight, tailOffset, transType)
    }

// 只适用于客户资料维护页面
    DC.ui.table_form = function(config) {
        var DC_table_form = function(config) {
            config = config || {}
            if (!config.el) {
                return
            }
            ...
            this.setMsg = function(target, msg) {
                ...
                $('.popup-error').html('<i class="fa fa-timDC-circle"></i> ' +  msg).show().parent().show()
            }
                ...
                for(var i = 0, trLen = trArray.length; i < trLen; i++) {
                    ...
                    for(var j = 0, inputLen = inputArray.length; j < inputLen; j++) {
                        ...
                        if (this.items[j].nonEmpty && val == '') {
                            this.setMsg(inputObj, DC.msg.get('non_empty'))
                            return false
                        }
                        ...
                        if(this.items[j].validate){
                            var input = DC.get('#tableConsigneeForm tbody tr:eq('+i+')').find('td:eq('+j+')').find('input')
                            var value = input.val()
        ...
        var ret = new DC_table_form(config)
        DC.ui.add(ret.id, ret)
        return ret
    }
    ...
    DC.mvx = {}
    DC.mvx.comps = {}
    DC.mvx.get = function(id) {
        return DC.mvx.comps[id]
    }
    DC.mvx.add = function(comp) {
        DC.mvx.comps[comp.id] = comp
    }
    DC.mvx.remove = function(id) {
        DC.mvx.comps[id] = undefined
        DC.mvx.comps[id] = null
        delete DC.mvx.comps[id]
    }
    DC.mvx.validate = function() {
        var rDC = true
        var map = DC.mvx.comps
        for (var i in map) {
